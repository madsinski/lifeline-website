-- =============================================================
-- list_all_companies: also return the contact person's phone number
-- so the admin companies card can show it under Contact.
--
-- phone lives encrypted (clients.phone_enc) — decrypt inline. The
-- function is SECURITY DEFINER so it has access to decrypt_text.
-- Returns NULL when the contact has no clients row or no phone yet.
-- =============================================================
...
begin
  ...
  return query
    select c.id,
           ...
           (select public.decrypt_text(cli.phone_enc) from public.clients cli where cli.id = c.contact_person_id) as contact_phone,
           c.created_at,
