# Biody Manager API — Technical Integration Description

**Prepared by:** Lifeline Health ehf.
**Audience:** Medalia engineering (leads + implementing engineer / coding agent).
**Purpose:** A complete, self-contained technical description of the Biody Manager API connection, so the Medalia team can integrate body-composition measurements into their own systems.
**Status:** Distilled from Lifeline Health's production integration. Every wire shape and quirk below was verified against Biody's live API. The "gotchas" in particular are hard-won and are **not** in Biody's official documentation.

> **Note on secrets.** This document deliberately contains **no live credentials** — service-account email/password, the HMAC signing secret, and project keys are delivered separately through a secure channel. Everything else needed to implement is here.
...
Lifeline Health integrated Biody using this exact pattern in 2025/2026 and shipped it to production. The integration replaces the standalone Biody clinician portal that patients otherwise log into separately. After the integration:

- Patients see body-composition history inside the app — no second login.
- Trends are correlated against everything else the app tracks (mood, sleep, exercise, blood markers).
...
Lifeline's integration took roughly **three engineer-weeks** end-to-end including the gotchas. With this guide a fresh team should land in **5–10 working days**:
...
## What you need from Biody up front

A short email to your Biody account manager (template in §11). Ask for:
...
Run a small worker (Edge Function, Lambda, container — any HTTPS-reachable service) that owns the Biody partner credential. The app never sees the credential or the JWT.

```
[App]  → POST /your-worker/create-patient → [Worker] → POST /api/patients     → [Biody]
...
- Trivial to swap to another provider (Polar Verity Sense, Tanita, etc.) later.
...
All paths relative to `${BASE}` = `https://www.biodymanager.com`. Every request carries `BM-Application: PartnerApp`; all except login also carry `Authorization: Bearer ${TOKEN}`.

| Method | Path | Purpose |
|---|---|---|
| `POST` | `/api/login_check` | Obtain JWT (body: `{email, password}`) |
| `POST` | `/${LOCALE}/api/patients?id=0` | Create patient |
| `GET`  | `/${LOCALE}/api/patients/{id}` | Fetch patient (uuid backfill) |
| `GET`  | `/${LOCALE}/api/patientgroups?limit=200&order_by[patientGroup.name]=asc` | List groups |
| `POST` | `/${LOCALE}/api/patientgroups?id=0` | Create group |
| `GET`  | `/${LOCALE}/api/notifications?filters[notification.controller]=measurement&filters[notification.read]=false&limit=100` | Unread measurement notifications |
| `PUT`  | `/${LOCALE}/api/notifications/{id}` | Mark notification read (body: `{read:true}`) |
| `GET`  | `/${LOCALE}/api/measurements/{id}` | Fetch single measurement (preferred) |
| `GET`  | `/${LOCALE}/api/measurements?filters[measurement.patient]={pid}&filters[measurement.status]=Analyzed&order_by[measurement.msrDate]=desc&limit=20` | List a patient's measurements (reconcile) |
| `POST` | `/api/interpret?controller=measurements&id={id}` | Trigger interpretation |

---

*Prepared by Lifeline Health ehf. Questions on anything in this document — contact Mads at Lifeline Health.*
