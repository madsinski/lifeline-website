...
export default function ApprovalsContent() {
  ...
  const [lectures, setLectures] = useState<PendingLecture[]>([]);
  const [loading, setLoading] = useState(true);
  const [busyId, setBusyId] = useState<string | null>(null);

  const load = useCallback(async () => {
    setLoading(true);
    const [{ data: evs }, { data: ivs }, { data: lecs }] = await Promise.all([
      supabase
      ...
      supabase
        .from("intro_lectures")
        .select("id, company_id, lecture_date, start_time, end_time, mode, location, companies:company_id(name)")
        .eq("approval_status", "requested")
    ...
    setLectures(((lecs as unknown[]) || []).map((e) => {
      const r = e as Record<string, unknown>;
      return { id: r.id as string, company_id: r.company_id as string, company_name: name(r.companies), lecture_date: r.lecture_date as string, start_time: r.start_time as string, end_time: r.end_time as string, mode: r.mode as "onsite" | "video", location: (r.location as string) ?? null };
    }));
    setLoading(false);
  }, []);
  ...
  const act = async (kind: "event" | "interview" | "lecture", id: string, action: "approve" | "reject") => {
    setBusyId(id);
    ...
    const url = kind === "event"
      ...
      : kind === "interview"
        ...
        : `/api/admin/business/intro-lectures/${id}/approve`;
  ...
  const totalPending = events.length + interviews.length + lectures.length;
    ...
    <div className="px-8 py-6 space-y-8 max-w-3xl">
      {loading ? (
        ...
        <>
          ...
          {lectures.length > 0 && (
            <section>
              <h2 className="font-semibold text-gray-900 mb-3">Introduction lectures ({lectures.length})</h2>
              <div className="space-y-2">
                {lectures.map((e) => (
                  <div key={e.id} className="flex items-center justify-between gap-4 bg-white border border-gray-200 rounded-lg p-4">
                    <div className="text-sm">
                      <div className="font-semibold text-gray-900">{e.company_name}</div>
                      <div className="text-gray-600">
                        {fmtDate(e.lecture_date)} · {e.start_time.slice(0,5)}–{e.end_time.slice(0,5)} ·{" "}
                        <span className="capitalize">{e.mode === "onsite" ? "On-site" : "Video/phone"}</span>
                        {e.mode === "onsite" && e.location ? ` · ${e.location}` : ""}
                      </div>
                    </div>
                    <div className="flex gap-2 shrink-0">
                      <button disabled={busyId === e.id} onClick={() => act("lecture", e.id, "approve")}
                        className="px-3 py-1.5 text-xs font-semibold text-white bg-emerald-600 rounded-lg hover:bg-emerald-700 disabled:opacity-40">Approve</button>
                      <button disabled={busyId === e.id} onClick={() => act("lecture", e.id, "reject")}
                        className="px-3 py-1.5 text-xs font-medium text-red-600 border border-red-200 rounded-lg hover:bg-red-50 disabled:opacity-40">Reject</button>
                    </div>
                  </div>
                ))}
