...
function normalizeClient(row: ClientRow, companyMap: Map<string, string>): Client {
  ...
  return {
    ...
    sex: (row as unknown as Record<string, unknown>).sex as string || "",
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        {activeTab === "clients" && (
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <a
              href="/admin/biody/bulk-create"
              className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-semibold border border-gray-300 text-gray-700 bg-white hover:bg-gray-50"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2h7M16 4v12m-6-8v8m-4-4h8" />
              </svg>
...
// Wellness-mode banner shown above the macro panel during the
// pre-Medalia-API interim. Reminds staff that the body comp + macro
// values shown here are the wellness-mode mirror — clinical decisions
// must be made against the sjúkraskrá in Medalia. Per Sprint 1.6.
function WellnessModeBanner() {
  return (
    <div className="rounded-lg border border-amber-200 bg-amber-50/60 px-4 py-3">
      <div className="flex items-start gap-2.5">
        ...
        <div className="text-xs text-amber-900 leading-relaxed">
          <p className="font-medium mb-0.5">Wellness-mode mirror — not the medical record</p>
          <p>
            Body composition and macro values here are the client&apos;s self-tracking copy.
            The clinical record (sjúkraskrá) is in Medalia. Open Medalia for blood test
            results, doctor&apos;s notes, and any treatment decisions. Coaching messages here
            must not contain clinical advice.
          </p>
        </div>
      </div>
    </div>
      ...
      >
        ...
        <td className="px-2 py-3" onClick={(e) => e.stopPropagation()}>
          <button onClick={() => onDeleteClick()}
            disabled={isDeleting}
            className="p-1 text-gray-300 hover:text-red-500 rounded-lg hover:bg-red-50 transition-colors disabled:opacity-50" title="Delete client">
