  ...
  return (
    <div ref={ref} style={{ position: "relative", width: "100%", aspectRatio: "16 / 9", borderRadius: 12, overflow: "hidden", background: "#000", boxShadow: "0 10px 40px -12px rgba(6,78,59,.25)" }}>
      ...
      >
        <DeckDefs />
        {slide
          ? <section className={`slide ${slide.theme}${brandClass(slide.brand)}${hasBg(slide) ? " has-bg" : ""}${bgStyleClass(slide)} active`}><SlideView slide={slide} zoomable /></section>
...
export function Deck({ slides, slidesIs, design, initialIndex = 0, onClose }: { slides: Slide[]; slidesIs?: Slide[]; design?: string; initialIndex?: number; onClose?: () => void }) {
  ...
  return (
    ...
    >
      ...
      {view.map((s, idx) => (
        <section key={s.id} className={`slide ${s.theme}${brandClass(s.brand)}${hasBg(s) ? " has-bg" : ""}${bgStyleClass(s)}${idx === i ? " active" : idx < i ? " prev" : ""}`}>
