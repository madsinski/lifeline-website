...
// POST { member_ids?: string[], all?: boolean, email?: boolean } → issue
...
export async function GET(req: NextRequest, ctx: { params: Promise<{ companyId: string }> }) {
  ...
  const { data: codes } = await supabaseAdmin
    ...
    .select("id, member_id, code, package_key, redeemed_at, expires_at, revoked_at, emailed_at, created_at")
  ...
  for (const m of targets) {
    for (let attempt = 0; attempt < 4; attempt++) {
      const { data, error } = await supabaseAdmin
        .from("company_hc_codes")
        .insert({ company_id: companyId, member_id: m.id, code: newCompanyCode(), expires_at: expires, created_by: a.user.id })
      ...
      const r = await sendEmail({
        ...
        html: renderBrandedEmail({
          ...
          bodyHtml: `<p style="margin:0 0 12px;">Hæ ${escapeHtml((m.full_name || "").split(" ")[0] || "")},</p>
            <p style="margin:0 0 12px;">${escapeHtml(a.company.name)} býður þér heilsufarsskoðun hjá Lifeline Health. Stofnaðu frían aðgang, veldu heilsufarsskoðunina og hakaðu við „Fyrirtækið mitt greiðir“ í greiðsluskrefinu. Sláðu þar inn kóðann:</p>
            ...
            <p style="margin:0;">Kóðinn er persónulegur og gildir í eitt ár. ${escapeHtml(a.company.name)} sér aldrei heilsufarsupplýsingar þínar.</p>`,
