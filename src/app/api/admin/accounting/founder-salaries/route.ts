...
export async function GET(req: NextRequest) {
  ...
  const byMonth = new Map<string, { amount_isk: number; note: string | null }>();
  for (const r of rowsRes.data || []) {
    byMonth.set(String(r.month).slice(0, 7), { amount_isk: Number(r.amount_isk) || 0, note: (r.note as string | null) ?? null });
  }
  const months = Array.from({ length: 12 }, (_, i) => {
    const key = `${year}-${String(i + 1).padStart(2, "0")}`;
    const hit = byMonth.get(key);
    return { month: key, amount_isk: hit?.amount_isk ?? 0, is_set: !!hit, note: hit?.note ?? null };
  });
...
export async function POST(req: NextRequest) {
  ...
  // null amount → clear the month (treated as 0 in the P&L).
  if (body.amount_isk === null || body.amount_isk === undefined) {
  ...
  const { error } = await supabaseAdmin
    .from("accounting_founder_salaries")
    .upsert({ month: monthDate, amount_isk: amount, note, updated_at: new Date().toISOString() }, { onConflict: "month" });
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
